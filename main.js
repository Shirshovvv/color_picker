...
class ColortableCustom extends HTMLElement {
    constructor() {
        super();
        this.innerHTML = `<div class="color__table-items"></div>`
      }
}
customElements.define("color-table", ColortableCustom);
...
const colortableWrapper = document.querySelector('.color__table-items')
const form = document.querySelector('.color__selector form')

const colortable = document.querySelector('.color__table')

// -----------------------------функции сохранения данных в localStorage-----------------------------------
function changeStorage(){
    let html = colortableWrapper.innerHTML.trim()
    if(html.length){
...
function setStorage(){
    ...
    colortableWrapper.innerHTML = localResult
}
function onDragLocalSave(){
    const dragItem = document.querySelectorAll('.color__table_item')
            dragItem.forEach(d=>{
...
const addColorBtn = document.querySelector('.color__table_btn button');
...
colorSelector.addEventListener('click', function(e){
    ...
    if(target.classList.contains('color__selector_btn')) {
        const changeColorItem = document.querySelector('.color__table_item.change_item')
        if(colorName.value == '') {
            validation(colorName)
            time(colorName)
        } else if (changeColorItem == null) {
            colortableWrapper.insertAdjacentHTML('beforeend', `
            <div class="color__table_item">
                <div class="color__table_item-body color"><div class="color__rect" style="background: #${colorCode.value}"></div></div>
                <div class="color__table_item-body name">${colorName.value}</div>
                <div class="color__table_item-body type">${colorType.innerHTML}</div>
                <div class="color__table_item-body colorCode">#${colorCode.value}</div>
                <div class="color__table_item-body changeBtn">
                ...
                <div class="color__table_item-body deleteBtn">
